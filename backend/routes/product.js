...
router.post('/add',auth.authenticateToken,role.checkRole,(req,res)=>{
...
router.patch('/update',auth.authenticateToken,role.checkRole,(req,res,next)=>{
    let product = req.body;
    ...
    connection.query(query,[product.name,product.categoryId,product.description,product.price,product.id],(err,results)=>{
        if(!err){
            if(results.affectedRows ==0){
                return res.status(400).json({message:"product id doesn't exist"});
            }
...
router.delete('/delete/:id',auth.authenticateToken,role.checkRole,(req,res,next)=>{
    const id = req.params.id;
    var query = "delete from product where id=?";
    connection.query(query,[id],(err,results)=>{
        if(!err){
            if(results.affectedRows ==0){
            return res.status(200).json({message:"product id doesn't exist"});
            }
